$(function () {
    ...
    if ($(".js-slider").length) {
        $(".js-slider").slick({
            ...
            prevArrow: '<a class="card-item__slider-arrow card-item__slider-arrow--prev" href="#"></a>',
            nextArrow: '<a class="card-item__slider-arrow card-item__slider-arrow--next" href="#"></a>',
            rows: 0,
        });
    }
		...
		element.slider({
			...
			change: function( event, ui ) {
				var values = ui.values, 
					...
					handle_right = $(this).find('.ui-slider-handle').last(),
					inputMin = $(this).closest('.form-range').find('input[type="hidden"]').first(),
					inputMax = $(this).closest('.form-range').find('input[type="hidden"]').last();
	...
	if( $('.js-selected-mother').length > 0 ){
		...
		$(document).on('click','.js-selected-mother__close', function (){
			deleteMother($(this), $(this).closest('.selected-mother__item').data('id'));
		});
		...
		$(document).on('click', ".js-mother-select", function (e){
			...
			if(!$(this).hasClass('card-selected')){
				if(!$('.selected-mother__item[data-id="' + $(this).data('id') + '"]').length) {
					addMother($(this));
				}
			}else {
	...
	$(document).on('click', '.js-add-mother', function(e) {
		...
		if(!$(this).hasClass('js-add-mother--disable')){
			if(!$('.selected-mother__item[data-id="' + $(this).data('id') + '"]').length) {
				addMother($(this));
				$('.mother-filter-result__wrapper .card[data-id="' + $(this).data('id') + '"]').find('.js-mother-select').addClass('card-selected');
			}
	...
	function addMother (block) {
		if($('.selected-mother__item--no-selected').length > 0){
			...
			element.find('.selected-mother__item-photo img').attr('src',block.data('image'));
	...
	if($('.js-phone-mask').length > 0){
		$('.js-phone-mask').inputmask("+7 (999) 999-99-99", {placeholder: "+7 (___) ___-__-__"});
	}
	...
	if($('ol[type="буквы"]').length > 0) {
		...
		[].forEach.call('абвгдежзиклмнопрстуфхцчшщэюя', (c, i) =>
			style.sheet.insertRule(`${list_russian_letter}>li:nth-child(${i+1})::before{content:"${c})"}`, 0)
		);
		style.sheet.insertRule(`${list_russian_letter}>li{list-style-type:none;position:relative}`, 0);
		style.sheet.insertRule(`${list_russian_letter}>li::before{position:absolute;width:2em;left:-2.25em;text-align:right;display:inline-block}`, 0);
	}
	/* list [type="буквы"] end */
	...
	if($('.js-add-social-input').length > 0) {
		$(document).on('click', '.js-add-social-input .nice-select li', function () {
			var type = $(this).data('value');
			var template = '<div class="col-12 col-sm-4"><div class="form__field form__field-social"><div class="form-input__wrap"><div class="form-input-icon form-input-icon--">'+ type +'</div><input type="text" class="form-input" placeholder=""></div></div></div>';
	...
	if($('.js-modal-video').length > 0) {
		$(document).on('click', '.js-modal-video', function (e){
			...
			$('.modal-video .modal-body').append('<iframe src="//www.youtube.com/embed/'+videoId+'?autoplay=1" scrolling="no" style="width: 100%; height: 100%;" allow="autoplay" frameborder="0">');
...
function videoplay(button) {
	...
	par.innerHTML = '<iframe src="//www.youtube.com/embed/'+videoId+'?autoplay=1" scrolling="no" style="width: 100%; height: 100%;" allow="autoplay" frameborder="0">';
}
